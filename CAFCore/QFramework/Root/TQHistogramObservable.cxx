...
/*@observable: [TQHistogramObservable] The TQHistogramObservable allows the mapping of values to
    other values based on an existing histogram (TH1, TH2, or TH3) stored in some ROOT file. It is
    automatically created whenn encountering expressions of the form 
    'TH1Map:myFile.root:myHist([observable1])' where TH1Map, TH2Map, or TH3Map are valid prefixes.
    Following the prefix and a collon the ROOT file containing the mapping histogram to be used and 
    the name of the histogram within the ROOT file are expected, again separated by a collon. Inside
    the then following parentheses a comma separated list of expressions is expected. These expressions
    are mapped to Subobservables and correspond to the axes of the histogram (in order x,y,z). The 
    TQHistogramObservable evaluates all Subobservables and finds the bin of the histogram that corresponds
    to these values. It then returns the value of that bin. An example usecase is given, for example, by some
    (initial) fake factor studies where events receive a weight based on the pT and eta of certain 
    particle candidates in the event.
*/
