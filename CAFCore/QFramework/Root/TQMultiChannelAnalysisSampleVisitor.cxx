...
int TQMultiChannelAnalysisSampleVisitor::visitFolder(TQSampleFolder * sampleFolder, TString&/*message*/) {
  ...
  if (this->checkVisitFriends(sampleFolder)){
    DEBUG("folder '%s' and all of its friends have been visited before...",sampleFolder->getPath().Data());
    return visitIGNORE;
  }
...
int TQMultiChannelAnalysisSampleVisitor::analyseTree(TQSample * sample, TString& message) {
  ...
  #ifdef HAS_XAOD
    TQToken* eventToken = sample->getEventToken();
    bool isxAOD = eventToken; //if we get an event token we must be running over xAODs...
    //we don't actually need the eventToken for anything else...
    sample->returnToken(eventToken);
  #endif
  ...
  for (auto sf : (*friends)) {
    ...
    DEBUGclass("retrieving number of entries");
    if (tree) { //we only need to care about adjusting the normalization due to limited number of events to be processed when there actually are events to process
