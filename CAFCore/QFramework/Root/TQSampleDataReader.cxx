...
void TQSampleDataReader::processHistogramOptions(TH1*& histo, TQTaggable* options){
  ...
  if ((rebinX > 0 || rebinY > 0 || rebinZ > 0)) {
    if(!TQHistogramUtils::rebin(histo,rebinX,rebinY,rebinZ,remap)){
      delete histo;
      histo = 0;
      setErrorMessage(__FUNCTION__,TString::Format(
                                                   "Invalid rebinning parameter for histogram '%s'", histo->GetName()));
    }
    if (f_Verbose > 1) {
  ...
  if (options->getTagDouble("~slope", slope)) {
    if (!TQHistogramUtils::applySlopeToHistogram(histo, slope)) {
      ...
      histo = 0;
      setErrorMessage(__FUNCTION__,TString::Format(
                                                   "Invalid slope parameter for histogram '%s'", histo->GetName()));
    }
  }
  ...
  if(labelsX.size() == (size_t)histo->GetXaxis()->GetNbins()){
  ...
  if(labelsY.size() == (size_t)histo->GetYaxis()->GetNbins()){  
  ...
  if(labelsY.size() == (size_t)histo->GetZaxis()->GetNbins()){    
...
int TQSampleDataReader::collectElementsWorker(TQSampleFolder * sampleFolder, const TString& elementName, const TString& subPath, TList* list, TList * baseScaleList, TList * sfList, TQTaggable * options, int indent) {
  ...
  //obj = dynamic_cast<T*>(sampleFolder->getObject(  )); 
...
//  for( const auto& pathTerm : paths ) {
//    std::cout << pathTerm.factor << ": " << pathTerm.path.Data() << std::endl;
//  }
//  for( const auto& pathTerm : elements ) {
//    std::cout << pathTerm.factor << ": " << pathTerm.path.Data() << std::endl;
//  }  
  
//  collectTimer->Start(false);
  TList * elementList = this->collectElements<CLASS>(paths, elements, subPath, sfList, options);
//  collectTimer->Stop();
//  collectTimer->Print();

  // INFOclass("Printing elementList:");
  // elementList->Print("",-1);

  if(elementList) {
    int nSubElements = 0;
    //    summationTimer->Start(false);
		nSubElements = this->sumElements(elementList,element,options);
//		summationTimer->Stop();
//    summationTimer->Print();
    elementList->SetOwner(true);
    delete elementList;
    //    std::cout << "list has " << nSubElements << " entries" << std::endl;    
    return nSubElements;
  } else {
