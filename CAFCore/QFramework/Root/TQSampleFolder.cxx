...
void TQSampleFolder::findFriendsInternal(TQSampleFolder* otherSF, bool forceUpdateSubfolder, std::shared_ptr<TList> otherSFContents){
  ...
  //TQSampleFolderIterator subitr(this->GetListOfFolders());
  while(itr.hasNext()){
    ...
    if (this->getDistToRoot() != other->getDistToRoot()) continue; //stricter requirement than above: folders must not only be unrelated or identical but must be identical or at same depth (which implies being unrelated) -> fixing synchronous initialization of friends being impossible/broken
    ...
    bool doBefriend = true;//subitr.hasNext(); //check if there is even an entry. TODO: if we assign true here as a default value we cause empty SampleFolders to become friends if they are at the same depth. I don't see any downside to this, but may require clarification. See also CAFCore#339 )
...
void TQSampleFolder::befriend(TQSampleFolder* other){
  ...
  if (!other) return;
  //std::cout<<"Befriending SFs at '"<<this->getPath().Data()<<"' and '"<<other->getPath().Data()<<"'"<<std::endl;
