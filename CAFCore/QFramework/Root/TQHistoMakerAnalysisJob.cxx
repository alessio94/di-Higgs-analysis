...
TString TQHistoMakerAnalysisJob::gDefaultAuthor = "TQHistoMakerAnalysisJob";
...
bool TQHistoMakerAnalysisJob::finalizeHistograms(){
  ...
  DEBUGclass("successfully created folder %s for cut %s", poolAt->getPath().Data(), getCut()->GetName());
  ...
  for (unsigned int i = 0; i < fHistograms.size(); i++) {
    ...
    if (h)
      {
        WARNclass("A histogram with the name '%s' is already in the sample folder. This can happen when you try to book the same histogram multiple times at the same cut. Please check your histogram file. Removing previous object for now.", h->GetName());
        folder->Remove(h);
      }
