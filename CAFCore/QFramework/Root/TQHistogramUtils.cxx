...
std::vector<int> TQHistogramUtils::getOptimizedBinBorders(TH1 * hsig, TH1 * hbkg, double minSignal, double minBkg, double maxBkgUnc,  bool significanceAgnostic, double maxSignal, double estimatedMaxSignInBin, bool verbose)
{
  ...
  while (movingUpperBin > 0) {
    ...
    for (int ibin = movingUpperBin; ibin >= 0; ibin--) {
      ...
      if (sig < minSignal || bkg < minBkg) {
        if (verbose) {INFO(TString::Format("Integral(lowBin=%d, upBin=%d): skipping because of insufficient signal yield (%f) or bkg yield (%f)", ibin, movingUpperBin, sig, bkg));}
        continue;
      }
      ...
      if (bkg != 0) {
        ...
        if (verbose) {INFO(TString::Format("Criteria satisfied at Integral(lowBin=%d, upBin=%d): sig=%f +- %f, bkg=%f +- %f, sigma=%f, sigmaUnc=%f, s/sqrt(b)=%f", ibin, movingUpperBin, sig, sig_err, bkg, bkg_err, sign, sign, sig / TMath::Sqrt(bkg)));}
...
double TQHistogramUtils::getSummedBinByBinSignificance(TH1 * histo_bkg, TH1 * histo_sig, TString options) {
  ...
  /* 2 and 3 dimensional histograms not yet supported */
  if (histo_bkg->InheritsFrom(TH3::Class()) || histo_bkg->InheritsFrom(TH2::Class()))
    return 0;
...
TH1 * TQHistogramUtils::normalize(TH1 * histo, const TH1 * normalizeToHisto) {
...
TH1 * TQHistogramUtils::getSmoothedVariation(const TH1 * h_nom, TH1 * h_var, const std::vector<int>& binBorders, bool keepOverFlowUnderFlowSeparate) {
  ...
  */
  ...
  if (binBorders.empty()) {
    DEBUGclass("No bin borders specified, returning varied histogram!");
    return h_var;
  }
  ...
  if (h_var_smoothed) {
    ...
    DEBUGclass("Couldn't return smoothed variation!")
  }
...
TH1* TQHistogramUtils::unrollHistogram(const TH2* input, bool firstX, bool includeUnderflowOverflow){
