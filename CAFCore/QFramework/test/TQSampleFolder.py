...
class TQSampleFolderTest(TQPythonTest):
    ...
    def test_copy_samplefolders(self):
        """
        Check if copying a folder preserves all elements and tags.
        """
        ### prepare
        folder = TQSampleFolder("base")
        b = folder.getSampleFolder("a/b+")
        c = TQSample("c")
        c.setTreeLocation("x:y")
        c.setNormalisation(10)
        b.addFolder(c)
        c.setTagBool("mytest",True)
        
        copy = folder.copy()
        self.assertTrue(copy)
        self.assertIsInstance(copy, QFramework.TQSampleFolder)
        self.assertTrue(copy.GetName() == "base")
        self.assertTrue(copy.getFolder("a/b/c"))
        self.assertIsInstance(copy.getFolder("a/b/c"), QFramework.TQSample)
        self.assertTrue(copy.getFolder("a/b/c").getTagBoolDefault("mytest",False))
        self.assertTrue(copy.getFolder("a/b/c").getTreeLocation() == "x:y")
        self.assertTrue(copy.getFolder("a/b/c").getNormalisation() == 10)
