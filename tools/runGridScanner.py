...
def loadGridScanResults(config, dictionary):
    ...
    if not results:
        QF.WARN(
            "cannot open file {:s}. Maybe you have forgotten to run the optimization first to produce the file".format(
                rootfname.Data()
            )
        )
...
def plotResults(config, dictionary):
    ...
    QF.INFO(
        "Choosing the figure of merit with the name {:s} for plotting".format(
            gridscanResults.FOMDefinitions[indexFOMForPlotting]
        )
    )
...
def createGridScanner(config, evaluator):
    ...
    if not lines:
        QF.BREAK(
            "Could not open input file with bounday list '{:s}'. Please check your configuration and try again. Aborting now...".format(
                boundaryFilePath
            )
        )
    ...
    if config.getTagBoolDefault("discardUnusedObservables", True):
        ...
        if config.getTagString("simple.axisToEvaluate", finalDiscriminantName):
            if not finalDiscriminantName in obsList:
                obsList.append(finalDiscriminantName)
            # Convert python list to ROOT TList to parse to gridscanner
        for obs in obsList:
            obsToScan.Add(ROOT.TObjString(obs.Data()))
    ...
    for l in lines:
        ...
        if isSplit:
            ...
            if len(evaluator.FOMDefinitions()) > 1:
                QF.ERROR(
                    "There is no support for multiple FOMs when a spliting cut is defined! Please choose one figure of merit and rerun!"
                )
                exit()
        else:
...
def runScan(config, sampleFolder, dictionary):
    ...
    QF.INFO(
        "Found sample folder with name '{:s}'".format(sampleFolder.getTagStringDefault(".key", sampleFolder.getName()))
    )
    ...
    QF.INFO(
        "Creating {:s} significance evaluator for channel {:s}".format(
            evalMode.Data(), dictionary.replaceInText("$(LEPCH)")
        )
    )
    ...
    if not config.getTagDefault("evaluator.verbose", False):
        print("")
    ...
    if nPoints < 1:
        QF.BREAK(
            "Something went terribly wrong - there are no points! one possible reason for this is that you requested to loop over non-existant variables. check your configuration and validate your input data. afterwards, you may try again."
        )
    ...
    QF.INFO(
        "Significances between {:f} and {:f}".format(
            results.points()[0].significance(), results.points()[nPoints - 1].significance()
        )
    )
...
def main(args):
    ...
    for ch in channels:
        ...
        if not results:
            ...
            QF.INFO(
                "Found previous gridscan results for channel "
                + ch
                + " here: "
                + str(rootfname)
                + ":results. Reading its contents now!"
            )
            QF.INFO("Top points are:")
            jobname = config.getTagDefault("nDimHistName", "gridscan")
            gridscanResults = results.getObject(jobname)
