...
parser = argparse.ArgumentParser(description="""A script to count the number of events in CxAODs/DxAODs.""")
parser.add_argument(
    "countNumberInDxAOD",
    metavar="countNumberInDxAOD",
    type=int,
    choices=[0, 1],
...
parser.add_argument(
    "validateLargeFiles",
    metavar="validateLargeFiles",
    type=int,
    choices=[0, 1],
    help="Validate the CollectionTree in large files." " Possible choices are 0/1.",
)
parser.add_argument(
    "doSummed",
    metavar="doSummed",
...
parser.add_argument("-s", "--safe", action="store_true", help="Fail on errors and some warnings (safe mode).")
parser.add_argument("-v", "--verbose", action="store_true", help="Verbose mode.")
example_usage = """
example usage:
  {0} 1 0 0 # to compare with AMI
  {0} 0 0 1 # to run the Reader""".format(
    __file__
)
try:
...
def main(argv):
    ...
    if not out_sample_list:
        print (
            "No sample list specified. Going to scan current directory for root "
            "files and then try to extract yields..."
        )
    else:
        print ("Trying to extract yields from files listed in '{}'".format(out_sample_list))
    ...
    if args.safe:
        for e in nrj:
            ...
            if os.path.isfile(out_file_name):
                print ("ERROR: Target file '{}' exists already. Exiting".format(out_file_name))
                sys.exit(7)
    ...
    if out_sample_list:
        if os.path.isdir(yieldDir):
            ...
            print ("WARNING: Directory '{}' does not exist, can't copy output files!".format(yieldDir))
            if args.safe:
                sys.exit(11)
    ...
    if copyFilesMD5:
        ...
        if os.path.isfile(out_file_md5):
            print ("File '{}' exists already, skipping yield counting.".format(out_file_md5))
            return
    ...
    for my_file in fileList:
        ...
        if len(list_outDSElement) == 8:
            ...
            print ("ERROR: list_outDSElement is in wrong format." " Can only handle 6 ot 8 elements.")
        ...
        if n_entries == 0:
            print "  WARNING: Zero events for file {}".format(my_file)
            if args.safe:
                file.Close()
                sys.exit(11)
        file.Close()
        ...
        frmt("Number of Entries (DxAOD)", n_entries)
    ...
    if writeDxAODAndCxAOD:
        for i_nrj in nrj:
            ...
            out_fileCxAOD.close()
            for i_count in countDxAOD:
                lineDxAOD = "%10s %10.0f %10.0f %15f %-30s" % (
                    i_count[0],
                    i_count[1],
                    i_count[2],
                    i_count[3],
