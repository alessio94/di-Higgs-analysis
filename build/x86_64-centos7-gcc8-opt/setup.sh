...
# CAFbbll when sourced from either BASH or ZSH.
#
...
ENV_VAR=CAFbbll_SET_UP
if [ "$*" = "extonly" ]; then
    ENV_VAR=CAFbbll_EXTONLY_SET_UP
elif [ "$*" = "relonly" ]; then
    ENV_VAR=CAFbbll_RELONLY_SET_UP
fi
...
if [ "x${ENV_VAR_VALUE}" = "x" ]; then
    ...
    export CAFbbll_DIR=$(\cd ${thisdir};\pwd)
    unset thisdir

    # Extract directory names relevant for the rest of the operations:
    export CAFbbll_PLATFORM=$(basename ${CAFbbll_DIR})
    export CAFbbll_VERSION=$(basename $(dirname $(dirname ${CAFbbll_DIR})))

    # Base projects used for the build of CAFbbll:
    CAFbbll_BASEPROJECTS=" AnalysisBase"

    # If the project has base projects, set up those first. First, we just set
    # up the external environment of the base projects:
    if [ "$*" != "relonly" ]; then
        for project in ${CAFbbll_BASEPROJECTS}; do
            # Strip off the whitespaces from the project name:
            p="$(echo ${project} | xargs)"
            # Name of the setup file in the base release:
            fname=${CAFbbll_DIR}/../../../../${p}/${CAFbbll_VERSION}/InstallArea/${CAFbbll_PLATFORM}/setup.sh
            if [ -f ${fname} ]; then
                . ${fname} extonly
            fi
            unset fname p
    ...
    if [ "$*" != "relonly" ]; then
        ...
        if [ -f ${CAFbbll_DIR}/env_setup.sh ]; then
            . ${CAFbbll_DIR}/env_setup.sh
        elif [ -f ${CAFbbll_DIR}/lcg_setup.sh ]; then
            . ${CAFbbll_DIR}/lcg_setup.sh
        fi
    fi
    ...
    if [ "$*" != "extonly" ]; then
        ...
        if [ -z "${CMAKE_PREFIX_PATH}" ]; then
            export CMAKE_PREFIX_PATH=${CAFbbll_DIR}:
        else
            export CMAKE_PREFIX_PATH=${CAFbbll_DIR}:${CMAKE_PREFIX_PATH}:
        fi

        # Set up the binary path(s):
        if [ -z "${PATH}" ]; then
            export PATH=${CAFbbll_DIR}/bin:
        else
            export PATH=${CAFbbll_DIR}/bin:${PATH}:
        fi
        export PATH=${CAFbbll_DIR}/share:${PATH}:

        # Set up the library path(s):
        if [ -z "${LD_LIBRARY_PATH}" ]; then
            export LD_LIBRARY_PATH=${CAFbbll_DIR}/lib:
        else
            export LD_LIBRARY_PATH=${CAFbbll_DIR}/lib:${LD_LIBRARY_PATH}:
        fi
        if [ -z "${DYLD_LIBRARY_PATH}" ]; then
            export DYLD_LIBRARY_PATH=${CAFbbll_DIR}/lib:
        else
            export DYLD_LIBRARY_PATH=${CAFbbll_DIR}/lib:${DYLD_LIBRARY_PATH}:
        fi

        # Set up the python path(s):
        if [ -z "${PYTHONPATH}" ]; then
            export PYTHONPATH=${CAFbbll_DIR}/lib:
        else
            export PYTHONPATH=${CAFbbll_DIR}/lib:${PYTHONPATH}:
        fi
        export PYTHONPATH=${CAFbbll_DIR}/python:${PYTHONPATH}:

        # Set up the jobOptions path:
        if [ -z "${JOBOPTSEARCHPATH}" ]; then
            export JOBOPTSEARCHPATH=${CAFbbll_DIR}/jobOptions:
        else
            export JOBOPTSEARCHPATH=${CAFbbll_DIR}/jobOptions:${JOBOPTSEARCHPATH}:
        fi

        # Set up the data path(s):
        if [ -z "${DATAPATH}" ]; then
            export DATAPATH=${CAFbbll_DIR}/share:
        else
            export DATAPATH=${CAFbbll_DIR}/share:${DATAPATH}:
        fi
        export DATAPATH=${CAFbbll_DIR}/data:${DATAPATH}:
        if [ -z "${CALIBPATH}" ]; then
            export CALIBPATH=${CAFbbll_DIR}/share:
        else
            export CALIBPATH=${CAFbbll_DIR}/share:${CALIBPATH}:
        fi
        export CALIBPATH=${CAFbbll_DIR}/data:${CALIBPATH}:

        # Set up the include path:
        if [ -z "${ROOT_INCLUDE_PATH}" ]; then
            export ROOT_INCLUDE_PATH=${CAFbbll_DIR}/include:
        else
            export ROOT_INCLUDE_PATH=${CAFbbll_DIR}/include:${ROOT_INCLUDE_PATH}:
        fi

        # Set up the XML path:
        if [ -z "${XMLPATH}" ]; then
            export XMLPATH=${CAFbbll_DIR}/XML:
        else
            export XMLPATH=${CAFbbll_DIR}/XML:${XMLPATH}:
        fi
...
# Clean up:
unset ENV_VAR
if [ ! -z "${CAFbbll_UNSETSHWORDSPLIT}" ]; then
    unsetopt shwordsplit
    unset CAFbbll_UNSETSHWORDSPLIT
fi
