...
EL::StatusCode PUReweightingTool::initialize(std::string campaign, bool isMC, int mcChannelNumber, bool isAFII)
{
    ...
    std::string ilumicalcFilesKey = "PRW_ilumicalcFiles_" + campaign;
    m_config.getif<std::vector<std::string>>(ilumicalcFilesKey, m_ilumicalcFiles);
    ...
    std::string actualMuFilesKey = "PRW_actualMuFiles_" + campaign;
    m_config.getif< std::vector<std::string> >(actualMuFilesKey, m_configFiles);
    if (isMC) {
        std::string configFiles_basepath = m_config.get<std::string>("PRW_configFiles_basepath");
        std::string simString = isAFII ? "AFII" : "FS";
        std::string prw_config_file_name = "pileup_" + campaign + "_dsid" + std::to_string(mcChannelNumber) + "_" + simString + ".root";
        m_configFiles.emplace_back(configFiles_basepath + "/" + prw_config_file_name);
    }
