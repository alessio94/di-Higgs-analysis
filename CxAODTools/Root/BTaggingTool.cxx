...
StatusCode BTaggingTool::get_tagjet_selection_vhbb(std::vector<const xAOD::Jet *> &signalJets,std::vector<const xAOD::Jet *> &selectedJets, 
  ...
  //useful to compute also nbtag. Used in both AllsignalJets and Leading2SignalJets
  for (int idx=0; idx<nSignalJet; ++idx) {
    const xAOD::Jet *Jet = signalJets.at(idx);
  ...
  if (strategy == "AllSignalJets") {
	...
	Error("tagjet_selection()", "AllIndexVector does not match nSignalJet. This should not happen.");
    }
  ...
  } // end tag strategy "AllSignalJets"
  ...
  if (strategy == "Leading2SignalJets") {
    ...
    if (lead_tag && sublead_tag && nbtag >= 2) tagcatExcl = 2;
  ...
  if( jetsToSort.size() > 0)
    {
      ...
      if (strategy == "AllSignalJets") 
	std::sort(jetsToSort.begin(), jetsToSort.end(), [] (const xAOD::Jet* jetA, const xAOD::Jet* jetB)
