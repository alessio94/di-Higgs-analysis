...
def extractIO(config, changes):
    ...
    if inconfigs:
        for inconfig in inconfigs:
            ...
            if filename:
                addinput.append(filename.split(":")[0])
    ...
    if outconfigs:
        for outconfig in outconfigs:
            ...
            if filename:
                addoutput.append(filename.split(":")[0])
...
def tasks_from_directory(args, configdir):
    ...
    for chunk in chunks:
        ...
        t = task(
            identifier=args.identifier + "_" + makeID(identifiers),
            args=args,
            setup=setup,
            outputs=outputs,
            inputs=inputs,
...
def single_task(args, config):
    ...
    if args.change:
        payload = payload + " --change " + " ".join(["'" + c + "'" for c in args.change])
    t = task(
        identifier=args.identifier, args=args, setup=setup, outputs=addoutputs, inputs=addinputs, payload=[payload]
    )
    return [t]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "directory", type=str, nargs="+", default=[], metavar="/path/to/jobs", help="directory of configs be submitted"
    )
    parser.add_argument(
        "--chunk",
        type=int,
        metavar="N",
        help="number of configs to be grouped into a single chunk and submitted as a job",
        default=1,
    )
    parser.add_argument(
        "--change", metavar="text", type=str, help="changes to apply to the input config file", nargs="*", default=[]
    )
    import QFramework
    import ROOT
