...
plot_options = dict()
plot_options["221"] = PlotOptions(title="2.2.1", line_color=kBlue, fill_style=3004, line_style=1)
plot_options["2211"] = PlotOptions(title="2.2.11", line_color=kRed, fill_style=3005, line_style=1)
...
def make_comp_plot(reader, campaign, channel, process, process_versions, cut, histo_name, output_dir, norm=False):
    ...
    process_name, process_title, process_path_221, process_path_2211 = process
    process_paths = dict()
    process_paths["221"] = process_path_221
    process_paths["2211"] = process_path_2211
    ...
    ratio_title.DrawLatex(0.05, 0.08, "2.2.11 / 2.2.1")
    canvas.Update()
...
def main(args):
    ...
    output_dir_norm = "results/zjets_comp/norm"
    if not os.path.exists(output_dir_norm):
        os.makedirs(output_dir_norm)

    output_dir_scale = "results/zjets_comp/scale"
    ...
    processes = list()
    processes.append(("zee", "Z #rightarrow ee", "Zee", "ee"))
    processes.append(("zmm", "Z #rightarrow #mu#mu", "Zmm", "mm"))
    processes.append(("ztt", "Z #rightarrow #tau#tau", "Ztt", "tautau"))

    versions = list()
    versions.append(("221", "/bkg/{channel}/{campaign}/{process}/main"))
    versions.append(("2211", "/oth/{channel}/{campaign}/Sh2211/Zjets/{process}"))

    cuts = ["Cut2TagMllSR1VBFVeto", "Cut2TagMbbSR2", "Cut2TagMbbZllCR"]
