    ...
    for process_name, process_path in processes:
        ...
        for cut_title, cut_name in cuts:
            ...
            if process_name == "Data":
                yields = int(counter.getCounter())
            else:
                if raw_counts:
                    yields = counter.getRawCounter()
                else:
                    yields = counter.getCounter()

            if cut_name == cuts[0][1]:
                reference_yields = yields

            acceptance = yields / reference_yields
            row.append("{:.3f}".format(acceptance * 100))
...
def main(args):
    ...
    processes = list()
    processes.append((r"$bbWW$ (Herwig 7)", "/sig/{channel}/{campaign}/nonres/bbWW"))
    processes.append((r"$bbWW$ (Pythia 8)", "/oth/{channel}/{campaign}/sig/nonres/pythia8/bbWW"))
    processes.append(("|", "|"))
    processes.append((r"$bb\tau\tau$ (Herwig 7)", "/sig/{channel}/{campaign}/nonres/bbtautau"))
    processes.append((r"$bb\tau\tau$ (Pythia 8)", "/oth/{channel}/{campaign}/sig/nonres/pythia8/bbtautau"))
    processes.append(("|", "|"))
    processes.append((r"$bbZZ$ (Herwig 7)", "/sig/{channel}/{campaign}/nonres/bbZZ"))
    processes.append((r"$bbZZ$ (Pythia 8)", "/oth/{channel}/{campaign}/sig/nonres/pythia8/bbZZ"))

    cuts = list()
    cuts.append((r"\vtop{\hbox{\strut CxAOD selection,}\hbox{\strut exactly 2 leptons}}", "CutApplyTTBarSplitting"))
    cuts.append(("Trigger", "CutTriggerMatching"))
    cuts.append(("2 $b$-jets", "CutNBJets2"))
    cuts.append(("Opposite charge leptons", "Cut2TagOS"))
    ...
    for version in ["raw", "weighted"]:
        ...
        output_dir = "results/cutflow_signal_systematics_acceptance_{:s}".format(version)
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="Create cutflow table with breakdown for different signal samples")
