...
from ROOT import kBlack, kRed, kBlue, kGray
...
plot_options = dict()
plot_options["fakes"] = PlotOptions(title="SSOS Fakes (default)", line_color=kBlack, fill_style=3004, line_style=1)
plot_options["fakes_0.8"] = PlotOptions(
    title="SSOS Fakes (0.8 x MC part)", line_color=kRed, fill_style=3005, line_style=1
)
plot_options["fakes_1.2"] = PlotOptions(
    title="SSOS Fakes (1.2 x MC part)", line_color=kBlue, fill_style=3005, line_style=1
)
...
def make_comp_plot(reader, campaign, channel, process_versions, cut, histo_name, output_dir, norm=False):
    logging.info("Plotting histogram %s for campaign %s, channel %s and cut %s", histo_name, campaign, channel, cut)

    is_nn_plot = histo_name.startswith("NN")
    ...
    if is_nn_plot:
        hists[versions[0]].SetMinimum(hist_min / 5)
        hists[versions[0]].SetMaximum(hist_max * 100)
    else:
    ...
    legend = TLegend(0.55, 0.70, 0.92, 0.92)
    ...
    ratios = [("fakes_0.8", "fakes"), ("fakes_1.2", "fakes")]
    ...
    ratio_title.DrawLatex(0.05, 0.08, "Modified / Default")
    canvas.Update()
...
def main(args):
    ...
    output_dir_scale = "results/fake_mc_uncertainty_check/scale"
    ...
    versions = list()
    versions.append(("fakes", "/bkg/{channel}/{campaign}/ddFakes/[data+mc/prompt]"))
    versions.append(("fakes_0.8", "/bkg/{channel}/{campaign}/ddFakes/[data+0.8*[mc/prompt]]"))
    versions.append(("fakes_1.2", "/bkg/{channel}/{campaign}/ddFakes/[data+1.2*[mc/prompt]]"))
    ...
    for campaign in campaigns:
        for channel in channels:
            for cut in cuts:
                ...
                for histo in histos:
                    if histo == "NN_SR1_Signal_Rebin_vs_Lepton1Pt":  # skip 2D Histos
                        continue
                    make_comp_plot(
                        reader,
                        campaign,
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(
        description="Check the impact of applying a 20% uncertainty of the MC part of the SSOS fake background"
    )
