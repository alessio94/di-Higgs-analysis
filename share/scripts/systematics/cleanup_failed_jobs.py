...
NTUPLE_FOLDER_PATH = "data/ntup/unmerged/systematics"
...
def delete_file(file_path, dryrun):
...
def clean_up_files(ntuple_file_name, ntuple_file_path, dryrun):
    ...
    delete_file(ntuple_file_path, dryrun)
    delete_file(sample_folder_path, dryrun)
...
def main(args):
    ...
    for ntuple_file_name in list_files_in_dir(ntuple_dir_path):
        ...
        if ntuple_file_size < FILE_SIZE_TRHESHOLD:
            ...
            clean_up_files(ntuple_file_name, ntuple_file_path, args.dryrun)


if __name__ == "__main__":
    PARSER = argparse.ArgumentParser("Clean up failed jobs for systematic ntuples")
    PARSER.add_argument("--dry-run", dest="dryrun", action="store_true", help="Don't delete the files", default=False)
