...
# taken from https://gitlab.cern.ch/atlas/athena/blob/21.2/PhysicsAnalysis/SUSYPhys/SUSYTools/scripts/AddZJetsWeights.py
# version: commit 23198a69
...
def main(infile, weightsFile, treename, attr_mcID, attr_nTruthJets):
    ...
    for iEvt in range(nEvts):
        ...
        if iEvt % 1000 == 0:
            print "\x1b[2K\r", "Current Event being Processed is: {0:15d}".format(iEvt),
            sys.stdout.flush()
        ...
        if (
            (361372 <= MCID <= 361443)
            or (363102 <= MCID <= 363122)
            or (363361 <= MCID <= 363363)
            or (363364 <= MCID <= 363411)
            or (364114 <= MCID <= 364127)
            or (364100 <= MCID <= 364113)
            or (364128 <= MCID <= 364141)
            or (364216 <= MCID <= 364217)
            or (364220 <= MCID <= 364221)
        ):
            decayType = "dilepton"

        elif (
            (361444 <= MCID <= 361467)
            or (363412 <= MCID <= 363435)
            or (364142 <= MCID <= 364155)
            or (366010 <= MCID <= 366035)
            or (364222 <= MCID <= 364223)
        ):
            decayType = "nolepton"

        elif (
            (361300 <= MCID <= 361371)
            or (363331 <= MCID <= 363354)
            or (363436 <= MCID <= 363459)
            or (363460 <= MCID <= 363483)
            or (364156 <= MCID <= 364169)
            or (364170 <= MCID <= 364183)
            or (364184 <= MCID <= 364197)
            or (364226 <= MCID <= 364227)
            or (364224 <= MCID <= 364225)
            or (364228 <= MCID <= 364229)
        ):
            decayType = "onelepton"
    ...
    print "\x1b[2K\r", "All entries processed"
    T.Write()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=__doc__,
        usage="\033[93m%(prog)s\033[0m file.root [options]",
        formatter_class=lambda prog: CustomFormatter(prog, max_help_position=30),
    )
    parser.add_argument("file", type=str, help="input ROOT tuple to reweight")
    parser.add_argument(
        "--weights",
        type=str,
        required=False,
        help="Input weights file to use for reweighting",
        default="/cvmfs/atlas.cern.ch/repo/sw/database/GroupData/dev/SUSYTools/Vjets_SysParameterization/VJetsWeights.root",
        metavar="",
    )
    parser.add_argument("--treename", type=str, required=False, help="Name of tree", default="nominal", metavar="")
    parser.add_argument(
        "--mcID", type=str, required=False, help="name of branch containing MC ID", default="mcID", metavar=""
    )
    parser.add_argument(
        "--nTruthJets",
        type=str,
        required=False,
        help="name of branch containing number of truth jets",
        default="nJets",
        metavar="",
    )
