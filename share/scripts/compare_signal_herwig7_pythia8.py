...
from ROOT import kRed, kBlue, kGray
...
plot_options = dict()
plot_options["herwig7"] = PlotOptions(title="Herwig 7", line_color=kBlue, fill_style=3004, line_style=1)
plot_options["pythia8"] = PlotOptions(title="Pythia 8", line_color=kRed, fill_style=3005, line_style=1)
...
def make_comp_plot(reader, campaign, channel, process, process_versions, cut, histo_name, output_dir, norm=False):
    logging.info("Plotting histogram %s for campaign %s, channel %s and cut %s", histo_name, campaign, channel, cut)
    process_name, process_title, process_path_herwig7, process_path_pythia8 = process
    process_paths = dict()
    process_paths["herwig7"] = process_path_herwig7
    process_paths["pythia8"] = process_path_pythia8
    ...
    for version, version_path in process_versions:
        process_path_formatted = version_path.format(
            campaign=caf_campaign, channel=caf_channel, process=process_paths[version]
        )
        versions.append(version)
        hists[version] = reader.getHistogram(process_path_formatted, "{}/{}".format(cut, histo_name))
        # print(process_path_formatted+", {}/{}".format(cut, histo_name))
        if not hists[version]:
            logging.error("Error retrieving histogram '%s/%s' from path '%s'", cut, histo_name, process_path_formatted)
            return

    plot_name = "{}-{}-{}".format(cut, histo_name, process_name)
    ...
    if cut == "Cut2TagMllSR1":
        ...
        cut_title = cut
    latex.DrawLatex(0.2, 0.62, "Region: " + cut_title)
    ...
    ratio_title.DrawLatex(0.05, 0.08, "Pythia 8 / Herwig 7")
    canvas.Update()
...
def main(args):
    ...
    output_dir_norm = "results/signal_systematics/norm"
    if not os.path.exists(output_dir_norm):
        os.makedirs(output_dir_norm)

    output_dir_scale = "results/signal_systematics/scale"
    ...
    processes = list()
    processes.append(("bbww", "HH #rightarrow bbWW", "bbWW", "bbWW"))
    processes.append(("bbtautau", "HH #rightarrow bb#tau#tau", "bbtautau", "bbtautau"))
    processes.append(("bbzz", "HH #rightarrow bbZZ", "bbZZ", "bbZZ"))

    versions = list()
    versions.append(("pythia8", "/sig/{channel}/{campaign}/nonres/{process}"))
    versions.append(("herwig7", "/sys/{channel}/{campaign}/sig/nonres/herwig7/{process}"))

    cuts = ["Cut2TagMllSR1VBFVeto"]
