...
def main(args):
    ...
    processes = list()
    processes.append((r"$bbWW$ (kl=1)", "/sig/{channel}/{campaign}/nonres/bbWW"))
    processes.append((r"$bbWW$ (kl=10)", "/sig/{channel}/{campaign}/kl10/bbWW"))
    processes.append(("|", "|"))
    processes.append((r"$bb\tau\tau$ (kl=1)", "/sig/{channel}/{campaign}/nonres/bbtautau"))
    processes.append((r"$bb\tau\tau$ (kl=10)", "/sig/{channel}/{campaign}/kl10/bbtautau"))
    processes.append(("|", "|"))
    processes.append((r"$bbZZ$ (kl=1)", "/sig/{channel}/{campaign}/nonres/bbZZ"))
    processes.append((r"$bbZZ$ (kl=10)", "/sig/{channel}/{campaign}/kl10/bbZZ"))
    ...
    for version in ["raw", "weighted"]:
        ...
        output_dir = "results/cutflow_signal_kl10_{:s}".format(version)
