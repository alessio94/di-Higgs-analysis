...
from ROOT import kGreen, kBlue, kGray, kOrange, kCyan
...
plot_options = dict()
plot_options["PflowLoose_FixedRad"] = PlotOptions(title="PflowLoose_FixedRad", line_color=kGreen + 2, fill_style=3005)
plot_options["PflowLoose_VarRad"] = PlotOptions(title="PflowLoose_VarRad", line_color=kBlue, fill_style=3004)
plot_options["PflowTight_FixedRad"] = PlotOptions(title="PflowTight_FixedRad", line_color=kOrange, fill_style=3005)
plot_options["PflowTight_VarRad"] = PlotOptions(title="PflowTight_VarRad", line_color=kCyan, fill_style=3004)


def style_histogram(hist, iso, pad_ratio, mode="plot"):
    title_size = 0.12
    lable_size = 0.12
    tick_length = 0.03
    hist.SetMarkerStyle(0)
    hist.SetLineColor(plot_options[iso].line_color)
    hist.SetLineWidth(1)
    ...
    if mode == "error":
        hist.SetFillColor(plot_options[iso].line_color)
        hist.SetFillStyle(plot_options[iso].fill_style)
    if mode == "ratio":
...
def make_comp_plot(readers, campaign, channel, process_info, cut_name, histo_name, output_dir, norm=False):
    ...
    for mu_iso in ISO:
        ...
        hists[mu_iso] = readers[mu_iso].getHistogram(path, "{}/{}".format(cut_name, histo_name))
        if not hists[mu_iso]:
            logging.error(
                "Error retrieving histogram '{}/{}' from path '{}' for '{}' ISO".format(
                    cut_name, histo_name, path, mu_iso
                )
            )
            return
    ...
    is_first = True
    for mu_iso in ISO:
        logging.debug("Plotting histogram %s for campaign %s and channel %s", mu_iso, campaign, channel)
        current_hist = hists[mu_iso]
        ...
        style_histogram(current_hist, mu_iso, pad_height_ratio)
        style_histogram(current_hist_error, mu_iso, pad_height_ratio, "error")

        hists[mu_iso] = current_hist

    hist_min, hist_max = get_hist_minmax(hists.values())
    hists[ISO[0]].SetMinimum(min(hist_min, 0))
    hists[ISO[0]].SetMaximum(hist_max / 0.7)
    if norm:
        hists[ISO[0]].GetYaxis().SetTitle("(1/N) Events")
    else:
        hists[ISO[0]].GetYaxis().SetTitle("Events")
    pad_upper.Update()
    ...
    legend = TLegend(0.60, 0.75, 0.92, 0.92)
    legend.SetBorderSize(0)
    legend.SetTextFont(42)
    legend.SetTextSize(0.04)
    # legend.SetFillColor(-1)
    for mu_iso in ISO:
        legend.AddEntry(hists[mu_iso], plot_options[mu_iso].title, "l")
    legend.Draw()
    ...
    ratio_title.DrawLatex(0.05, 0.08, "Variation / PflowLoose_VarRad")
    canvas.Update()
...
def main(args):
    ...
    output_dir_norm = "results/muon_iso_study/muons_norm"
    if not os.path.exists(output_dir_norm):
        os.makedirs(output_dir_norm)

    output_dir_scale = "results/muon_iso_study/muons_scale"
    ...
    histo_patterns = dict()
    histo_patterns["sf"] = [
        "Cut2TagMllSR1VBFVeto/Lepton0*",
        "Cut2TagMllSR1VBFVeto/Lepton1*",
        "Cut2TagMbbSR2/Lepton0*",
        "Cut2TagMbbSR2/Lepton1*",
    ]
    histo_patterns["df"] = ["Cut2TagMllSR1VBFVeto/Lepton0*", "Cut2TagMllSR1VBFVeto/Lepton1*"]

    processes = list()
    processes.append(("signal", "Signal", "/sig/{channel}/{campaign}/nonres"))
    processes.append(("background", "Bkg.", "/bkg/{channel}/{campaign}/[prompt+ddFakes/[data+mc/prompt]]"))

    for campaign in campaigns:
        for channel in channels:
            for process_info in processes:
                for histo in histos:
                    if check_histo_match(str(histo.GetString()), histo_patterns[channel]):
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser("Compare plots for different muon isolation WPs")
    PARSER.add_argument(
