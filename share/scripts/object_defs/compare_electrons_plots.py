...
plot_options = dict()
plot_options["Medium_FixedCutLoose"] = PlotOptions(title="Medium, FixedCutLoose", line_color=kRed, fill_style=3004)
plot_options["Medium_Gradient"] = PlotOptions(title="Medium, Gradient", line_color=kGreen + 2, fill_style=3005)
plot_options["Tight_FixedCutLoose"] = PlotOptions(title="Tight, FixedCutLoose", line_color=kBlue, fill_style=3004)
plot_options["Tight_Gradient"] = PlotOptions(title="Tight, Gradient", line_color=kBlack, fill_style=3005)
...
def make_comp_plot(readers, campaign, channel, process_info, cut_name, histo_name, output_dir, norm=False):
    ...
    for ele_id in ID:
        for ele_iso in ISO:
            identifier = "{}_{}".format(ele_id, ele_iso)
            versions.append(identifier)
            path = process_path.format(channel=caf_channel, campaign=caf_campaign)
            hists[identifier] = readers[identifier].getHistogram(path, "{}/{}".format(cut_name, histo_name))
            if not hists[identifier]:
                logging.error(
                    "Error retrieving histogram '{}/{}' from path '{}' for '{}' ID and '{}' ISO".format(
                        cut_name, histo_name, path, ele_id, ele_iso
                    )
                )
                return
    ...
    legend = TLegend(0.65, 0.75, 0.92, 0.92)
    ...
    ratios = [
        ("Medium_Gradient", "Medium_FixedCutLoose"),
        ("Tight_FixedCutLoose", "Medium_FixedCutLoose"),
        ("Tight_Gradient", "Medium_FixedCutLoose"),
    ]
    ...
    ratio_title.DrawLatex(0.05, 0.08, "Variation / Medium, FixedCutLoose")
    canvas.Update()
...
def main(args):
    ...
    for ele_id in ID:
        for ele_iso in ISO:
            ...
            identifier = "{}_{}".format(ele_id, ele_iso)
            samples[identifier] = sample_folder
            readers[identifier] = TQSampleDataReader(sample_folder)

    output_dir_norm = "results/object_defs/electrons_norm"
    if not os.path.exists(output_dir_norm):
        os.makedirs(output_dir_norm)

    output_dir_scale = "results/object_defs/electrons_scale"
    ...
    histo_patterns = ["*CutMll/*Lepton0*", "*CutMll/*Lepton1*"]

    processes = list()
    processes.append(("signal", "Signal", "/sig/{channel}/{campaign}/nonres"))
    processes.append(
        ("background", "Total background", "/bkg/{channel}/{campaign}-/bkg/{channel}/{campaign}/top/ttbar/dilepton")
    )
