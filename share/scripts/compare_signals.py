...
from ROOT import kRed, kGreen, kAzure, kCyan, kGray
...
plot_options = dict()
plot_options["bbww"] = PlotOptions(title="bbWW", line_color=kGreen, fill_style=3004, line_style=1)
plot_options["bbtautau"] = PlotOptions(title="bb#tau#tau", line_color=kRed, fill_style=3005, line_style=1)
plot_options["bbzz_2l2v"] = PlotOptions(title="bbZZ (2l2v)", line_color=kAzure, fill_style=3004, line_style=1)
plot_options["bbzz_2l2q"] = PlotOptions(title="bbZZ (2l2q)", line_color=kCyan, fill_style=3005, line_style=1)
...
def make_comp_plot(reader, campaign, channel, processes, cut, histo_name, output_dir, norm=False):
    logging.info("Plotting histogram %s for campaign %s, channel %s and cut %s", histo_name, campaign, channel, cut)
    caf_campaign = str_to_caf_campaign(campaign)
    caf_channel = str_to_caf_channel(channel)

    hists = dict()
    versions = list()
    for process_name, process_title, process_path in processes:
        process_path_formatted = process_path.format(campaign=caf_campaign, channel=caf_channel)
        versions.append(process_name)
        hists[process_name] = reader.getHistogram(process_path_formatted, "{}/{}".format(cut, histo_name))
        if not hists[process_name]:
            logging.error("Error retrieving histogram '%s/%s' from path '%s'", cut, histo_name, process_path_formatted)
            return

    plot_name = "{}-{}".format(cut, histo_name)
    ...
    ratio_title.DrawLatex(0.05, 0.08, "bbZZ (2l2q) / bbZZ (2l2v)")
    canvas.Update()
...
def get_list_of_histograms(histos, cut, patterns):
...
def main(args):
    ...
    output_dir_norm = "results/signal_comparison/norm"
    if not os.path.exists(output_dir_norm):
        os.makedirs(output_dir_norm)

    output_dir_scale = "results/signal_comparison/scale"
    ...
    processes = list()
    processes.append(("bbww", "HH #rightarrow bbWW", "/sig/{channel}/{campaign}/nonres/bbWW"))
    processes.append(("bbtautau", "HH #rightarrow bb#tau#tau", "/sig/{channel}/{campaign}/nonres/bbtautau"))
    processes.append(("bbzz_2l2v", "HH #rightarrow bbZZ (2l2v)", "/sig/{channel}/{campaign}/nonres/bbZZ"))
    processes.append(
        ("bbzz_2l2q", "HH #rightarrow bbZZ (2l2q)", "/oth/{channel}/{campaign}/sig/nonres/herwig/bbZZ_2l2q")
    )

    cuts = ["Cut2TagMllSR1", "Cut2TagMbbSR2", "Cut2TagMbbZllCR"]
    ...
    for campaign in campaigns:
        for channel in channels:
            for cut in cuts:
                ...
                histo_patterns = ["*"]
                histos = get_list_of_histograms(histos, cut, histo_patterns)
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="Compare signal samples showered with Herwig 7 and Pythia 8")
