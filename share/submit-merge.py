...
def makePreMergeTaskList(args_, preMergeDir, setup):
    ...
    if args.maxFiles == -1:
        args.maxFiles = int(math.ceil(math.sqrt(len(inputFiles))))
    ...
    for thisFile in inputFiles:
        ...
        if not partLabel in jobFilesDict.keys():
            jobFilesDict[partLabel] = []
    ...
    for key in jobFilesDict.keys():
        ...
        payload = "tqmerge "
        for thisFile in jobFilesDict[key]:
            payload = payload + thisFile + " "
...
def makeFinalMergeTask(args_, retList, setup):
    ...
    payload = "tqmerge "
    for thisPreMergedName in preMergedNames:
        payload = payload + thisPreMergedName + " "
    payload = payload + "--output " + os.path.realpath(args.output) + " "
    if args.traceid:
        payload = payload + " -t " + args.traceid
    logFile = os.path.join(args.logpath, args.identifier + ".final.log")
    finalTask = submit.task(
        args.identifier + ".final",
        payload,
        setup=setup,
        memory=args.memoryFinal,
        time=args.time,
        inputs=preMergedNames,
        outputs=[os.path.realpath(args.output)],
        dependencies=retList,
        logFile=logFile,
        errFile=logFile,
    )
...
def main(args):
    ...
    if allDone:
        print "All jobs are done. You may wish to delete the temporary output by calling 'rm -r ", preMergeDir, "'"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--memory", default="4000", type=float, help="memory requested per premerge job", required=False
    )
    parser.add_argument(
        "--memoryFinal", default="8000", type=float, help="memory requested for final job", required=False
    )
    ...
    parser.add_argument(
        ...
        dest="traceid",
        default="asv",
        help="trace ID of the analysis sample visitor",
    )
    parser.add_argument(
        "-m",
        "--downmerge",
        ...
        dest="downmerge",
        default=None,
        help="tag prefix to trigger downmerging",
    )
    parser.add_argument(
        "-p",
        "--patch",
        ...
        default=[],
        action="append",
        help="a patch style file to apply to every component",
    )
    parser.add_argument(
        "-s",
        "--sfname",
        ...
        dest="sfname",
        default="samples",
        help="name of the sample folder being merged",
    )
    parser.add_argument(
        "-d", "--depth", metavar="DEPTH", type=int, default=2, dest="depth", help="output folder split depth"
    )
    parser.add_argument(
        "-noSum", dest="sum", action="store_false", help="sum elements in TQFolders (counters, histograms)"
    )
    parser.add_argument(
        "-Sum", dest="sum", action="store_true", help="sum elements in TQFolders (counters, histograms)"
    )
